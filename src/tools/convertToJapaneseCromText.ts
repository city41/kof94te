...
function generateAssembly(
  ...
  for (let i = 0; i < tiles.length; ++i) {
    ...
    if (isControlTileOutput(tile)) {
      ...
      const newTileIndex = dedupedTiles.findIndex((t) => t.char === tile.char);
...
async function main() {
  ...
  for (const input of inputSpec.inputs) {
    ...
    partOutputs.push({
      destAsm: input.destAsm,
      tiles: currentPartTileOutput,
    });
  ...
  const dedupedTiles = uniqBy(totalTileOutput, (tile) => {
    return tile.char;
  });

  const cromTiles = generateCromTiles(dedupedTiles, paletteCanvasContext);
  fsp.writeFile(
  ...
  for (const partOutput of partOutputs) {
    const assembly = generateAssembly(partOutput.tiles, dedupedTiles);
    fsp.writeFile(partOutput.destAsm, assembly);
  }
}
