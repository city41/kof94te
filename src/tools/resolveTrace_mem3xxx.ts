...
function dumpMemory(registerMap: RegisterMap): string {
  return Object.entries(registerMap).reduce((accum, e, i) => {
    if (!e[0].startsWith("3")) {
      return accum;
    }
    ...
    return `${accum} ${e[0]}=${e[1].toString(16)}${nl}`;
  }, "");
}
...
function resolveLine(
  ...
  const finalResolvedLineWithMemory = `${finalResolvedLine}\n  ${dumpMemory(
    registerMap
  )}\n`;

  return {
    resolvedLine: finalResolvedLineWithMemory,
