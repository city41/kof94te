...
function should_deny(si)
	local offset = si - 0x71d6

	if offset >= 0 and (offset % 0x20) == 0 and (offset / 0x20) < 12 then
		return true
	end
...
function on_vram_write(offset, data)
	...
	if offset == REG_VRAMRW then
		...
		if next_vram_index >= FIX_LAYER and next_vram_index <= SCB2 and should_deny(next_vram_index) then
			return 0xff
		end

		-- if (data & 0xfff) >= 0x343 and (data & 0xfff) <= 0x354 then
		-- 	print(string.format("denying: %x at %x", data, next_vram_index))
		-- 	return 0xff
		-- end
